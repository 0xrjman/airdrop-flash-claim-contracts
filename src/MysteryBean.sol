...
interface IBeanRedeemer {
    function redeemBeans(
        address to,
        uint256[] calldata beanIds
...
contract MysteryBean is ERC2981, MultisigOwnable, OperatorFilterer, ERC721A {
    ...
    function _mintWrapperNoSafeReceiverCheck(
        address to,
        uint256 amount
    ) private {
        uint256 numBatches = amount / MINT_BATCH_SIZE;
    ...
    function getCanClaims(
        uint256[] calldata azukiIds
